...
class LoginController extends Controller
{
    ...
    */
    ...
    {
        if (
            config('instance.limit_users_active.enabled') &&
            SessionService::getTotalActiveSessions() >= (int) config('instance.limit_users_active.max_users_active')
        ) {
    ...
    protected function loginBlocked(User $user): bool
    {
        return in_array(
            $user->status,
            [
                StatusEnums::DELETE_QUEUE,
                StatusEnums::DELETED,
                StatusEnums::SUSPENDED,
            ],
            true
        );
    }
    ...
    protected function authenticated($request, $user): void
    {
        if (config('instance.limit_users_active.enabled')) {
            SessionService::setActiveSession($request->session()->getId(), $user->id);
        }
    ...
    {
        if (config('instance.limit_users_active.enabled')) {
            SessionService::removeActiveSession($request->session()->getId());
        }
