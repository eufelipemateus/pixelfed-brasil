...
class StatusUser extends Command
{
    ...
    protected function authDiagnostics(User $user): void
    {
        ...
        $status = $user->status;
        if ($status === StatusEnums::ACTIVE) {
            $ok[] = 'status = null (active)';
        } elseif (in_array($status, [StatusEnums::DELETED, StatusEnums::DELETE_QUEUE], true)) {
            $problems[] = 'status = "'.$status->value().'" (treated as deleted account)';
        } else {
            $problems[] = 'status = "'.$status->value().'" (non-active status may restrict the account)';
        }
    ...
    protected function diagnoseMissingUser(string $id): void
    {
        ...
        if ($similar->isNotEmpty()) {
            ...
            foreach ($similar as $u) {
                $rows[] = [$u->id, $u->username, $u->email, $u->status?->value() ?? 'null', $u->deleted_at ?? 'null'];
            }
